...
function databasePopulate {
	...
	/bin/echo "Creating users and roles for DC2D..."
	OUT=`/usr/bin/mysql ${DB_NAME} -u $DB_USER -p$DB_PASSWORD <create_roles_d.sql`
	if [ -n "$OUT" ]
	then
...
/bin/echo "If you will be asked for password please use mysql user password."
if [ -z "$DB_KEEP" ];
then
...
#
# creating test users in the system
#
echo "Creating test users in local system"
pass=$(perl -e 'print crypt($ARGV[0], "password")' "password")
sudo adduser ictintersect1 -p $pass
sudo adduser ictintersect2 -p $pass
sudo adduser ictintersect3 -p $pass
sudo adduser ictintersect4 -p $pass
...
cp -r ${HOME_DIR}/staging/_ssh/* ${TOMCAT6_HOME}/mda-data/_ssh
chown -R tomcat ${TOMCAT6_HOME}/mda-data
